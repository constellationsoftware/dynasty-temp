class Salary < ActiveRecord::Base
    ...
    scope :by_position, Proc.new { |filter = nil|
        if !(filter.nil?) and filter.size > 0
            ...
            where("SUBSTRING(position, 1, 2) IN (?)", POSITION_PRIORITIES).order(order_by_position_priority)
        end
    }
    scope :with_valid, lambda {
        select('DISTINCT salaries.*, p.id IS NULL AS is_valid').joins('LEFT OUTER JOIN picks p ON salaries.id = p.person_id')
    }
    scope :by_rating, order('points DESC')
    scope :available, joins('LEFT OUTER JOIN picks p ON salaries.id = p.person_id').where('p.id IS NULL')
