...
json.lineup     game.player_points_for(team) do |json, player_score|
    json.position       player_score.lineup.position.abbreviation.upcase + (!(player_score.player_point_id.nil?) && player_score.lineup.flex ? "(#{player_score.player.position.abbreviation.upcase})" : '')
